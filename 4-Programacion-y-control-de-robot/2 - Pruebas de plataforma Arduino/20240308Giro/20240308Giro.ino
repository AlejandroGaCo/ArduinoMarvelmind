...
void loop() {
  ...
  if(millis() - t1 > period){
    ...
    float errorPosX, errorPosY, errorAngZ, centerX, centerY, centerAngle; //Variables de error

    centerX = (ActualR.x + ActualL.x)/2;
    centerY = (ActualR.y + ActualL.y)/2;
    centerAngle = atan2(ActualR.y - ActualL.y, ActualR.x - ActualL.x)*180/PI;


    errorPosX = Destino.x - ActualR.x;
    errorPosY = Destino.y - ActualR.y;
    errorAngZ = Destino.angle - centerAngle;
...
//
//    Serial.print("Angle: calculated=");
//    Serial.println(centerAngle, res);

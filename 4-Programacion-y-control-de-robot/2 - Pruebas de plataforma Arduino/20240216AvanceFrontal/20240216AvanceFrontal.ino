...
const unsigned long period = 50; //tasa de refresco en mseg
...
void loop() {
  ...
  if(millis() - t1 > period){
    ...
    if (errorPosY > 0.1){
      ...
      digitalWrite(X2, LOW);
      Serial.print("Avanzo hacia enfrente");
    } else if (errorPosY < -0.1){
      digitalWrite(X1, LOW);
      digitalWrite(X2, HIGH);
      Serial.print("Avanzo hacia atras");
    } else {
      digitalWrite(X1, LOW);
      digitalWrite(X2, LOW);
